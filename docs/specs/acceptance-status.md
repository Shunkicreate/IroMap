...
- FR-1: `達成`
  - 根拠: 3Dパネルに RGB/HSL/Lab のタブ切替を追加し、表示空間に応じて再描画
- FR-2: `達成`
  - 根拠: 各色空間表示でドラッグ回転を共通処理で提供
- FR-3: `達成`
  - 根拠: 各色空間表示の hover/click が既存 Inspector/Color Copy 連携を維持
- FR-4: `達成`
  - 根拠: RGB表示時のみ既存スライス平面オーバーレイを表示し、Slice連携を維持
- FR-5: `達成`
  - 根拠: タブの選択状態を専用スタイルで強調し、選択中タブを明確化
- FR-6: `達成`
  - 根拠: 3D軸ガイド（軸線/軸ラベル）を追加し、チェックボックスで表示OFF可能
- FR-7: `達成`
  - 根拠: Slice パネルに固定軸と2D X/Y軸ラベルを表示し、軸切替で追従
- FR-8: `達成`
  - 根拠: RGBキューブと Slice をデスクトップで横並び配置し、モバイルで縦並び化
- FR-9: `達成`
  - 根拠: キューブサイズ調整スライダーを追加し、表示ON/OFFを提供
- FR-10: `達成`
  - 根拠: 上下ドラッグの回転計算を反転し、操作方向を補正
