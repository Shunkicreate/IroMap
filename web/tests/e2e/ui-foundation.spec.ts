...
test("T-105(ui-foundation): 写真分析からキューブ確認導線で色選択できる", async ({ page }) => {
  await page.goto("/");
  await uploadRedPng(page);
  await page.getByRole("button", { name: "キューブで確認" }).first().click();
...
test("T-107(ui-foundation): 操作通知がaria-liveで提供されることを確認", async ({ page }) => {
  ...
  await expect(liveRegion).toContainText("ファイル名=red.png");
});
